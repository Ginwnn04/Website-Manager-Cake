...
const LIST_PROVINCE = "listProvince";



function pushDataToLocal() {
...
function pushDataProvinceToLocal() {
  if (localStorage.getItem(LIST_PROVINCE) === null) {
    getDataProvince();
  }
}
...
async function getDataProvince() {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      localStorage.setItem(LIST_PROVINCE, JSON.stringify(listProvince));
...
window.onload = pushDataToLocal();
window.onload = pushAccountToLocal();
window.onload = pushDataProvinceToLocal();
